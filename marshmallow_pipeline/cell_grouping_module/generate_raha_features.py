...
def _strategy_runner_process(self, args):
    ...
    if self.VERBOSE:
        logging.info("%s cells are detected by %s", len(detected_cells_list), strategy_name)
...
def run_strategies(self, d, char_set):
    ...
    if not self.STRATEGY_FILTERING:
        if os.path.exists(sp_folder_path):
            ...
            pool.close()
            pool.join()
            logging.info("%%%%%%%%%%%%%%%%%%%%%%All strategies are run on the dataset.%%%%%%%%%%%%%%%%%%%%%%")
    else:
...
def generate_features(self, d, char_set_dict):
    ...
    for j in range(d.dataframe.shape[1]):
        ...
        for strategy_index, strategy_name in enumerate(sorted_strategy_profiles):
            logging.info("******************************Generating features for strategy: %s", strategy_name)
        ...
        if "TFIDF" in self.ERROR_DETECTION_ALGORITHMS:
            ...
            try:
                ...
                feature_vectors = np.column_stack((feature_vectors, np.array(tfidf_features.todense())))
            except:
                pass
        ...
        if self.VERBOSE:
            logging.info("%s Features are generated for column %s", feature_vectors.shape[1], j)
        # Adding headers hash
