...
def test_init(df_n_labels, output_path, all_cell_clusters_records, cell_cluster_cells_dict_all, manual_labeling_done, n_cores, save_mediate_res_on_disk):
...
def cell_cluster_sampling_labeling(cell_clustering_df, cell_cluster_cells_dict, n_cores, manual_labeling_done, output_path, table_cluster, col_cluster):
    logging.info(
    ...
    try:
        if cell_clustering_df["n_labels_updated"].values[0] > 0:
            ...
            if not manual_labeling_done:
                samples_dict, cell_clustering_df = sampling(cell_clustering_df, X_temp, y_temp, value_temp, n_cores, datacells_uids)
...
def error_detector(
    ...
    with multiprocessing.Pool(initializer=test_init, initargs=(df_n_labels, output_path, all_cell_clusters_records, cell_cluster_cells_dict_all, manual_labeling_done, n_cores, save_mediate_res_on_disk)) as pool:
        original_data_keys = []
...
def test(col_cluster, table_cluster):
    ...
    cell_cluster_sampling_labeling_dict = cell_cluster_sampling_labeling(
        cell_clustering_df, cell_cluster_cells_dict, n_cores, manual_labeling_done, output_path, table_cluster, col_cluster
    )
    if not manual_labeling_done:
        return None
    if save_mediate_res_on_disk:
