    ...
    for i in range(0, total_cols - 1):
        if (
            ...
            for j in range(i + 1, total_cols):
                if (
                    genFunc.getColumnType(inputTable.iloc[:, j].tolist()) == 1
                ):  # the subject in rdf triple should be a text column
                    mergeRelSem = {}
    ...
    for i in range(0, total_cols - 1):
        ...
        if genFunc.getColumnType(input_table.iloc[:, i].tolist()) == 1:
            ...
            for j in range(i + 1, total_cols):
                ...
                # logging.info(column_pairs)
                # assign relation semantic to each value pair of i and j
                for k in range(0, unique_rows_in_pair):
                    # logging.info(k)
                    # extract subject and object
                    found_relation = 0
                    subject_value = genFunc.preprocessString(
                        str(column_pairs.iloc[k][0]).lower()
                    )
                    object_value = genFunc.preprocessString(
                        str(column_pairs.iloc[k][1]).lower()
                    )
                                ...
                                if obj_entities != "None":
                                    ...
                                    for sub_entity in sub_entities:
                                        for obj_entity in obj_entities:
                                            ...
                                            entity_forward = (
                                                sub_entity + "__" + obj_entity
                                            )
                                            entity_backward = (
                                                obj_entity + "__" + sub_entity
                                            )
                                            relation_forward = FACT_DICT.get(
                                                entity_forward, "None"
                                            )
                                            relation_backward = FACT_DICT.get(
                                                entity_backward, "None"
                                            )
                                            if relation_forward != "None":
                                                ...
                                                if key not in entities_finding_relation:
                                                    entities_finding_relation[key] = {
                                                        sub_entity
                                                    }
                                                else:
                                                    entities_finding_relation[key].add(
                                                        sub_entity
                                                    )
                                                ...
                                                for s in relation_forward:
                                                    if s in semantic_dict_forward:
                                                        semantic_dict_forward[
                                                            s
                                                        ] += 1  # relation semantics in forward direction
                                                    else:
                                            ...
                                            if relation_backward != "None":
                                                ...
                                                for s in relation_backward:
                                                    if s in semantic_dict_backward:
                                                        semantic_dict_backward[
                                                            s
                                                        ] += 1  # relation semantics in reverse direction
                                                    else:
                                                        semantic_dict_backward[s] = 1
                if len(semantic_dict_forward) > 0:
                    relation_bag_of_words.append(
                        (
                            max(semantic_dict_forward, key=semantic_dict_forward.get)
                            + "-r",
                            str(i) + "_" + str(j),
                    ...
                    relation_dictionary[str(i) + "-" + str(j)] = [
                        (
                            max(semantic_dict_forward, key=semantic_dict_forward.get),
                            max(semantic_dict_forward.values()) / total_kb_forward_hits,
                        )
                    ]
                    total_hits[str(tab_id) + "-" + str(i) + "-" + str(j)] = (
                        total_kb_forward_hits,
                        unique_rows_in_pair,
                    )
                if len(semantic_dict_backward) > 0:
                    relation_bag_of_words.append(
                        (
                            max(semantic_dict_backward, key=semantic_dict_backward.get)
                            + "-r",
                            str(j) + "_" + str(i),
                            max(semantic_dict_backward.values())
                            / total_kb_backward_hits,
                        )
                    )
                    relation_dependencies.append(str(j) + "-" + str(i))
                    relation_dictionary[str(j) + "-" + str(i)] = [
                        (
                            max(semantic_dict_backward, key=semantic_dict_backward.get),
                            max(semantic_dict_backward.values())
                            / total_kb_backward_hits,
                    ...
                    total_hits[str(tab_id) + "-" + str(j) + "-" + str(i)] = (
                        total_kb_backward_hits,
                        unique_rows_in_pair,
                    )
    return (
...
def computeColumnSemantics(
    ...
    for columnName, columnData in input_table.items():
        if (
            genFunc.getColumnType(input_table[columnName].tolist()) == 1
        ):  # check column Type
            input_table[columnName] = input_table[columnName].map(str)
            ...
            all_top_types = [
                v
                for v in sorted(all_found_types.items(), key=lambda kv: (-kv[1], kv[0]))
                if v[0] in CLASS_DICT
            ]
            if all_top_types:
                ...
                if scoring_function == 1:
                    column_bag_of_words.append(
                        (
                            selected_top_type + "-" + label,
                            str(col_id),
                            all_found_types[selected_top_type] / total_kb_hits,
                        )
                    )
                    column_dictionary[int(col_id)] = [
                        (
                            selected_top_type,
                            all_found_types[selected_top_type] / total_kb_hits,
                        )
                    ...
                    for each in all_found_types:
                        if (
                            each in children_of_top_types
                            and (all_found_types[each] / top_type_count) >= 0
                        ):
                            column_bag_of_words.append(
                                (
                                    each + "-c",
                                    str(col_id),
                                    all_found_types[each] / total_kb_hits,
                                )
                            )
                            if int(col_id) not in column_dictionary:
                                column_dictionary[int(col_id)] = [
                                    (each, all_found_types[each] / total_kb_hits)
                                ]
                            else:
                                column_dictionary[int(col_id)].append(
                                    (each, all_found_types[each] / total_kb_hits)
                                )
        col_id += 1
...
def main(which_benchmark, which_mode):
    ...
    GROUND_TRUTH_PATH = (
        r"marshmallow_pipeline/santos/groundtruth/"
        + current_benchmark
        + "UnionBenchmark.pickle"
    )
    SUBJECT_COL_PATH = (
        r"marshmallow_pipeline/santos/groundtruth/"
        + current_benchmark
        + "IntentColumnBenchmark.pickle"
    )
    ...
    SYNTH_TYPE_LOOKUP_PATH = (
        r"../hashmap/" + current_benchmark + "_synth_type_lookup." + pickle_extension
    )
    SYNTH_RELATION_LOOKUP_PATH = (
        r"../hashmap/"
        + current_benchmark
    ...
    SYNTH_TYPE_INVERTED_INDEX_PATH = (
        r"marshmallow_pipeline/santos/hashmap/"
        + current_benchmark
        + "_synth_type_inverted_index."
        + pickle_extension
    )
    SYNTH_RELATION_INVERTED_INDEX_PATH = (
        r"marshmallow_pipeline/santos/hashmap/"
        + current_benchmark
    ...
    MAP_PATH = (
        ...
        + current_benchmark
        + "_benchmark_result_by_santos_"
        + current_mode
        + ".csv"
    )
    FINAL_RESULT_PICKLE_PATH = (
        r"marshmallow_pipeline/santos/stats/"
        + current_benchmark
        + "_benchmark_result_by_santos_"
        + current_mode
    ...
    TRUE_RESULTS_PATH = (
        r"marshmallow_pipeline/santos/stats/"
        + current_benchmark
        + "_benchmark_true_result_by_santos_"
        + current_mode
        + ".csv"
    )
    FALSE_RESULTS_PATH = (
        r"marshmallow_pipeline/santos/stats/"
        + current_benchmark
        + "_benchmark_false_result_by_santos_"
        + current_mode
        + ".csv"
    )
    QUERY_TIME_PATH = (
        r"marshmallow_pipeline/santos/stats/"
        + current_benchmark
        + "_benchmark_query_time_by_santos_"
        + current_mode
        + ".csv"
    )
    ...
    if which_mode == 1 or which_mode == 3:
        ...
        yago_inverted_index = genFunc.loadDictionaryFromPickleFile(
            YAGO_MAIN_INVERTED_INDEX_PATH
        )
        yago_relation_index = genFunc.loadDictionaryFromPickleFile(
            YAGO_MAIN_RELATION_INDEX_PATH
        )
        main_index_triples = genFunc.loadDictionaryFromPickleFile(
            YAGO_MAIN_PICKLE_TRIPLE_INDEX_PATH
        )
    else:
        label_dict = {}
    ...
    if which_mode == 2 or which_mode == 3:
        ...
        synth_type_inverted_index = genFunc.loadDictionaryFromPickleFile(
            SYNTH_TYPE_INVERTED_INDEX_PATH
        )
        synth_relation_inverted_index = genFunc.loadDictionaryFromPickleFile(
            SYNTH_RELATION_INVERTED_INDEX_PATH
        )
    else:
        synth_type_kb = {}
    ...
    logging.info(f"Time taken to load benchmarks in seconds: {difference}")
    logging.info("-----------------------------------------")
    scoring_function = 2
    ...
    for table in query_table_file_name:
        ...
        logging.info(f"Processing Table number: {total_queries}")
        ...
        if which_mode == 2 or which_mode == 3:
            (
                ...
                synth_subject_semantics,
            ) = computeSynthRelation(input_table, subject_index, synth_relation_kb)
            synthetic_column_dictionary = computeSynthColumnSemantics(
                input_table, synth_type_kb
            )
                            ...
                            for subject_item in subject_type:
                                for object_item in object_type:
                                    ...
                                    if str(i) + "-" + str(j) in current_relations:
                                        relation_tuple_forward = (
                                            relation_dictionary.get(
                                                str(i) + "-" + str(j), "None"
                                            )
                                        )
                                    if str(j) + "-" + str(i) in current_relations:
                                        relation_tuple_backward = (
                                            relation_dictionary.get(
                                                str(j) + "-" + str(i), "None"
                                            )
                                        )
                                    ...
                                    if relation_tuple_forward != "None":
                                        ...
                                        if triple_dict_key in query_table_triples:
                                            if (
                                                triple_score
                                                > query_table_triples[triple_dict_key][
                                                    0
                                                ]
                                            ):
                                                query_table_triples[triple_dict_key] = (
            ...
            if len(synthetic_triples_dictionary[key]) > 0:
                ...
                for synthetic_triple in synthetic_triples:
                    ...
                    if synthetic_triple_name in synth_query_table_triples:
                        if (
                            synthetic_triple_score
                            > synth_query_table_triples[synthetic_triple_name][0]
                        ):
                            synth_query_table_triples[synthetic_triple_name] = (
                                synthetic_triple_score,
                                key,
                            )
                    else:
        ...
        for item in query_table_triples:
            matching_tables = main_index_triples.get(
                item[0], "None"
            )  # checks yago inv4erted index
                    ...
                    if (dlt_name, col_pairs) not in already_used_column:
                        ...
                        if already_used_column[(dlt_name, col_pairs)] > total_score:
                            ...
                            if dlt_name not in table_count_final:
                                ...
                                table_count_final[dlt_name] -= already_used_column[
                                    (dlt_name, col_pairs)
                                ]
                                table_count_final[dlt_name] += total_score
                            already_used_column[(dlt_name, col_pairs)] = total_score
        ...
        for item in synth_query_table_triples:
            matching_tables = synth_relation_inverted_index.get(
                item[0], "None"
            )  # checks synth KB index
            if matching_tables != "None":
                triple = item[0]
                ...
                for data_lake_table in matching_tables:
                    ...
                    if (dlt_name, dlt_col1) in synth_type_inverted_index:
                        dlt_col1_contents = synth_type_inverted_index[
                            (dlt_name, dlt_col1)
                        ]
                    if (dlt_name, dlt_col2) in synth_type_inverted_index:
                        dlt_col2_contents = synth_type_inverted_index[
                            (dlt_name, dlt_col2)
                        ]
                    ...
                    if col_pairs + "-" + dlt_col1 + "-" + dlt_col2 in synth_col_scores:
                        ...
                        match_keys_22 = (
                            dlt_col2_contents.keys() & query_col2_contents.keys()
                        )
        ...
        sortedTableList = sorted(
            table_count_final.items(), key=lambda x: x[1], reverse=True
        )
        logging.info(
            f"***************sortedTableList****************** {sortedTableList}"
        )
        for key in sortedTableList:
            if not G.has_node(key[0]):
                G.add_node(key[0])
            if key[0] != table_name:
