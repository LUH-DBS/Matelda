            ...
            for each in all_found_types:
                if each in children_of_top_types and each in TYPE_SCORE_DICT:
                    ...
                    column_bag_of_words.append(
                        (each + "-c", str(col_id), current_type_score / total_kb_hits)
                    )
                    if int(col_id) in column_dictionary:
                        column_dictionary[int(col_id)].append(
                            (each, current_type_score / total_kb_hits)
                        )
                    else:
...
def main(which_benchmark):
    ...
    logging.info(
        f"Time taken to load yago in seconds: {int(yago_loading_end_time - yago_loading_start_time)}"
    )
    ...
    for table in DATA_LAKE_TABLES:
        try:
            ...
            logging.info("------------------------------")
            logging.info(f"Processing table: {tab_id}")
            table_name = table.rsplit("/", 1)[-1]
            logging.info(f"Table path: {table}")
            ...
            (
                ...
                relation_dependencies,
                relation_dictionary,
            ) = computeRelationSemantics(input_table, tab_id, label_dict, fact_dict)
            column_tuple_bag_of_words, column_dictionary = computeColumnSemantics(
                input_table,
                tab_id,
                label_dict,
                type_dict,
            ...
            # logging.info(max(column_dictionary.keys()))
                                ...
                                for subject_item in subject_type:
                                    for object_item in object_type:
                                        ...
                                        if relation_tuple_forward != "None":
                                            ...
                                            relation_score = relation_tuple_forward[0][
                                                1
                                            ]
                                            triple_dict_key = (
                                                subject_name
                                            ...
                                            if (
                                                triple_dict_key
                                                not in main_index_triples
                                            ):
                                                main_index_triples[triple_dict_key] = {
                                                    table_name: triple_score
                                                }
                                            else:
                                                current_tables = main_index_triples[
                                                    triple_dict_key
                                                ]
                                        ...
                                        if relation_tuple_backward != "None":
                                            relation_name = relation_tuple_backward[0][
                                                0
                                            ]
                                            relation_score = relation_tuple_backward[0][
                                                1
                                            ]
                                            triple_dict_key = (
                                                object_name
            ...
            for semantics in relation_tuple_bag_of_words:
                if semantics[0] not in main_inverted_index:
                    main_inverted_index[semantics[0]] = [
                        (table_name, semantics[1], semantics[2])
                    ]
                else:
                    main_inverted_index[semantics[0]].append(
                        (table_name, semantics[1], semantics[2])
                    )
            ...
            individual_time[table_name] = (
                individual_time_stop - individual_time_start
            ) / 10**9
            # logging.info(relation_tuple_bag_of_words)
            # logging.info(column_tuple_bag_of_words)
    ...
    logging.info(
        f"Time taken to process all tables in seconds: {int(yago_processing_end_time - yago_processing_start_time) / 10**9}"
    )
    logging.info(f"Number of ignored tables: {ignored_table}")
    logging.info(f"Tables not found in KB: {not_found_in_kb}")
    genFunc.saveDictionaryAsPickleFile(
        main_inverted_index, YAGO_MAIN_INVERTED_INDEX_PATH
    )
    genFunc.saveDictionaryAsPickleFile(
        main_relation_index, YAGO_MAIN_RELATION_INDEX_PATH
    )
    main_index_triples = convertDictToList(main_index_triples)
    genFunc.saveDictionaryAsPickleFile(
        main_index_triples, YAGO_MAIN_PICKLE_TRIPLE_INDEX_PATH
    )
