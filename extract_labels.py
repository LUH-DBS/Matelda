...
def generate_table_ground_truth(row) -> List[Tuple[int, int, int, bool]]:
    ...
    clean_df = pd.read_csv(row.clean_path, sep=",", header="infer", encoding="utf-8",
    ...
    for rowIndex, row2 in labels_df.iterrows(): #iterate over rows
        for columnIndex, value in row2.items():
            tables_ground_truth.append((row.table_id, labels_df.columns.get_loc(columnIndex), rowIndex, value))
            # pyspark cant safe a list in a Dataframe (type infering problem)
    return tables_ground_truth


def generate_labels_pyspark(csv_paths_df: DataFrame, labels_path: str, extract_labels_enabled: bool) -> DataFrame:
